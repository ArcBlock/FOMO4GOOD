...
Verified network profiles: Arc **testnet** (`config/staging-campaign.json`) and Arc **mainnet** (`config/mainnet-campaign.json`, chain 5042, recipient `0x985F6a6B60a0fFeaEC463EB7D912502e539F06eA`). Local loopback collection can be opened with `FOMO_MODE=testnet` or `mainnet`, `HOST=127.0.0.1`, and `FOMO_ACCEPT_REAL=1` on the companion Node service; browse `http://localhost:4931/arc/` (not the ARC page port). Staging Workers collect on testnet; production Workers collect on mainnet. See [docs/operations.md](operations.md).
